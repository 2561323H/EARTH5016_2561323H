...
while t <= tend
    ...
    while resnorm > tol
        ...
        u   = - KDx (diff(p,1,1)/h);  % x-speed
        w   = - KDz (diff(p,1,2)/h - Drho * g0);  % z-speed

        % get pressure residual (Div.v = 0)
        res_p = - (diff(u,1,1)+diff(w,1,2));
...
upos = 0.5*(u + abs(u));    % positive velocity
uneg = 0.5*(u - abs(u));    % negative velocity

wpos = 0.5*(w + abs(w));    % positive velocity
wneg = 0.5*(w - abs(w));    % negative velocity

% get values on stencil nodes for x
fxmmm = f(ix(1:end-6));
fxmm  = f(ix(2:end-5));
fxm   = f(ix(3:end-4)); 
fxc   = f(ix(4:end-3)); 
...
% get values on stencil nodes for z
fzmmm = f(iz(1:end-6));
fzmm  = f(iz(2:end-5));
fzm   = f(iz(3:end-4)); 
fzc   = f(iz(4:end-3)); 
...
switch ADVN
    ...
    case 'CFD2'
        ...
        fzppos = (fzc+fzp)./2;     fzpneg = fzppos;
        fzmpos = (fzc+fzm)./2;     fzmneg = fzmpos;
...
% calculate flux balance for rate of change
div_qxpos = upos.*(fxppos - fxmpos)/dx;
div_qxneg = uneg.*(fxpneg - fxmneg)/dx;
div_qx    = div_qxpos + div_qxneg;

div_qzpos = upos.*(fzppos - fzmpos)/dx;
div_qzneg = uneg.*(fzpneg - fzmneg)/dx;
